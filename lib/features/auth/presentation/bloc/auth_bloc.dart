...
class AuthBloc extends Bloc<AuthEvents, AuthStates> {
  ...
  AuthBloc({required this.signupUsecase, required this.loginUsecase, required this.pref}) : super(UnAuthenticatedState()) {
  ...
  Future<void> _onCheckAuthStatusEvent(CheckAuthStatusEvent event, Emitter<AuthStates> emit) async {
    ...
    if (accessToken != null && refreshToken != null) {
      emit(AuthenticatedState(logedInUser: null));
    } else {
  ...
  Future<void> _onSignUpEvent(SignUpEvent event, Emitter<AuthStates> emit) async {
    ...
    result.fold((failure) => emit(AuthErrorState(message: failure.message)), (user) => emit(AuthenticatedState(logedInUser: user)));
  }
